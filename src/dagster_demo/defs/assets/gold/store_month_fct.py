...
def gold_store_month_fact(
    context: dg.AssetExecutionContext,
    lidlo_de_silver_month_fact: pl.LazyFrame,
    carretwo_fr_silver_month_fact: pl.LazyFrame,
) -> pl.LazyFrame:
    """Union silver assets and partition by retailer."""
    df = gold_store_fact_processing(
        context, [lidlo_de_silver_month_fact, carretwo_fr_silver_month_fact], "month"
    )
    return df


@dg.asset_check(asset=gold_store_month_fact, blocking=True)
def gold_store_month_fact_schema_check(
    context: dg.AssetCheckExecutionContext, gold_store_month_fact: pl.LazyFrame
):
    ...
    check_results = check_polars_schema(
        df_schema=gold_store_month_fact.collect_schema(),
