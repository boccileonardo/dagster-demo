...
@dg.asset_check(asset=lidlo_de_silver_day_fact, blocking=True)
def silver_day_fact_schema_check(
    context: dg.AssetCheckExecutionContext, lidlo_de_silver_day_fact: pl.LazyFrame
):
    """Validate table schema:
    - Error if primary keys missing.
    - Error on wrong types.
    ...
    """
    check_results = check_polars_schema(
        df_schema=lidlo_de_silver_day_fact.collect_schema(),
...
@dg.asset_check(asset=lidlo_de_silver_week_fact, blocking=True)
def silver_week_fact_schema_check(
    context: dg.AssetCheckExecutionContext, lidlo_de_silver_week_fact: pl.LazyFrame
):
    """Validate table schema:
    - Error if primary keys missing.
    - Error on wrong types.
    ...
    """
    check_results = check_polars_schema(
        df_schema=lidlo_de_silver_week_fact.collect_schema(),
        expected_schema=store_fact_pl_schema,
        required_cols_list=store_fact_required_cols,
    )
    return dg.AssetCheckResult(passed=check_results["passed"], metadata=check_results)
...
@dg.asset_check(asset=lidlo_de_silver_month_fact, blocking=True)
def silver_month_fact_schema_check(
    context: dg.AssetCheckExecutionContext, lidlo_de_silver_month_fact: pl.LazyFrame
):
    """Validate table schema:
    - Error if primary keys missing.
    - Error on wrong types.
    ...
    """
    check_results = check_polars_schema(
        df_schema=lidlo_de_silver_month_fact.collect_schema(),
        expected_schema=store_fact_pl_schema,
        required_cols_list=store_fact_required_cols,
    )
    return dg.AssetCheckResult(passed=check_results["passed"], metadata=check_results)
